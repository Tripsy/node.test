...
class UserPermissionController {
    public create = asyncHandler(async (req: Request, res: Response) => {
        ...
        await Promise.all(
            validated.data.permission_ids.map(async (permission_id: number) => {
                const existingUserPermission = await UserPermissionRepository.createQuery()
                ...
                if (existingUserPermission) {
                    if (existingUserPermission.deleted_at) {
                        ...
                        await UserPermissionRepository.restore(existingUserPermission.id);
                        ...
                        results.push({
                            permission_id: permission_id,
                            message: lang('user_permission.error.already_exists')
                        });
                    }
                } else {
    ...
    public find = asyncHandler(async (req: Request, res: Response) => {
        ...
        const withDeleted: boolean = policy.allowDeleted() && validated.data.filter.is_deleted;
        ...
        const [entries, total] = await UserPermissionRepository.createQuery()
            ...
            .filterBy('permission.operation', validated.data.filter.operation, 'LIKE')
