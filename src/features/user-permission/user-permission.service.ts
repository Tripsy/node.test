...
export class UserPermissionService {
	 ...
	 */
	public async create(data: UserPermissionValidatorCreateDto, user_id: number) {
        return Promise.all(
            data.permission_ids.map(async (permission_id) => {
                ...
                if (existingUserPermission) {
                    if (existingUserPermission.deleted_at) {
                        await this.repository.restore(existingUserPermission.id);
	...
	public findByFilter(data: UserPermissionValidatorFindDto, user_id: number, withDeleted: boolean) {
