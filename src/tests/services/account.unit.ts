...
jest.mock('@/lib/providers/email.provider', () => ({
    loadEmailTemplate: jest.fn(() => Promise.resolve({
        language: 'en',
        content: {
    ...
    describe('setupToken', () => {
        it('should create token and save to repository', async () => {
            ...
            const mockRequest = createRequest({
                ...
                get: function (header: string) {
                    if (header.toLowerCase() === 'user-agent') return 'Mozilla/5.0';
                    return '';
                }
            });

            // Act
            const result = await accountService.setupToken(mockUser, mockRequest as any);
        ...
        it('should include proper metadata from request', async () => {
            ...
            const mockRequest = createRequest({
                ...
                get: (header: string) => header === 'user-agent' ? 'Custom Agent' : '',
            });
